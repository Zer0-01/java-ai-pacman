 ...
 */
public class Test3 extends Controller<MOVE>
{	
	...
	{			
		...
		int[] powerPills2 = game.getPowerPillIndices();
		int minDistancePower = Integer.MAX_VALUE;
		int minPowerPills = 0;
		
		for(int powerPills : powerPills2) {
			int distance = game.getShortestPathDistance(current, game.getPowerPillIndex(powerPills));
			
			if(distance<minDistancePower) {
				minDistancePower = distance;
				minPowerPills=powerPills;
			}
			
		}
		if(game.getShortestPathDistance(current, minPowerPills)<20) {
			return game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(),game.getPowerPillIndex(minPowerPills),DM.PATH);
		}
