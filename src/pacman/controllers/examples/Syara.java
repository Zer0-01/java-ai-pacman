 ...
 */
public class Syara extends Controller<MOVE>
{	
	...
	{			
		...
		//Strategy 3: go after the nearest pill that could save Pac-Man from being eaten by a ghost
		...
		// Find the closest pill to Pac-Man
		int closestPill = game.getClosestNodeIndexFromNodeIndex(current, targetsArray, DM.PATH);
		
		// Check if there is a nearby ghost that could reach Pac-Man before he reaches the closest pill
		for(GHOST ghost : GHOST.values())
		{
			if(game.getGhostEdibleTime(ghost) == 0 && game.getGhostLairTime(ghost) == 0)
			{
				int distanceToGhost = game.getShortestPathDistance(current, game.getGhostCurrentNodeIndex(ghost));
				int distanceToPill = game.getShortestPathDistance(closestPill, game.getGhostCurrentNodeIndex(ghost));
				
				if(distanceToGhost < distanceToPill)
				{
					// If a ghost could reach Pac-Man before he reaches the closest pill, run away from it
					return game.getNextMoveAwayFromTarget(current, game.getGhostCurrentNodeIndex(ghost), DM.PATH);
				}
			}
		}
		// If no nearby ghost poses a threat, go towards the closest pill
