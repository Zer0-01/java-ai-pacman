 ...
 */
public class HoldUntilGhostNear extends Controller<MOVE>
{	
	...
	{			
		...
		for(int i=0;i<pills.length;i++)					//check which pills are available			
			if(game.isPillStillAvailable(i) && (canEatGhost || !hasPowerPills))
				targets.add(pills[i]);
		...
		int target = game.getClosestNodeIndexFromNodeIndex(current,targetsArray,DM.PATH);
		double distance = game.getDistance(current, target, DM.PATH);
		if (hasPowerPills && distance < MIN_DISTANCE/3) {
			for(GHOST ghost : GHOST.values())
				if(game.getGhostEdibleTime(ghost)==0 && game.getGhostLairTime(ghost)==0)
					if(game.getShortestPathDistance(current,game.getGhostCurrentNodeIndex(ghost))<MIN_DISTANCE) 
						return game.getNextMoveTowardsTarget(current,target,DM.PATH);
			return game.getNextMoveTowardsTarget(current,current+10,DM.PATH);
		}
